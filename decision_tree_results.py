...
def decision_tree_max_depth_cross_validation_with_api_call(max_depth, number_of_chunks, chunk_number):
    ...
    pipe = Pipeline(y=rate, features=features, model=decision_tree, balance=True, bootstrap=True, normalisation=True,
                    cross_valid=True, n_chunks=number_of_chunks, chunk_n=chunk_number)
...
def decision_tree_max_depth_cross_validation(rate, features, max_depth, number_of_chunks, chunk_number):
    ...
    pipe = Pipeline(y=rate, features=features, model=decision_tree, bootstrap=False, cross_valid=True,
                    n_chunks=number_of_chunks, chunk_n=chunk_number)
...
def run_test_depth_range_cross_valid(rate, features, depth_range, number_of_chunks):
    ...
    for i in range(1, depth_range + 1):
        ...
        for j in range(number_of_chunks):
            performance_results_handler = decision_tree_max_depth_cross_validation(rate, features, i, number_of_chunks,
                                                                                   j)
...
def test_w_print_stats(depth_range, number_of_chunks):
    ...
    tree_depth_nested, r2_value_nested, validation_mse_nested, training_mse_nested = run_test_depth_range_cross_valid(
        rate, features, depth_range, number_of_chunks)
    ...
    for metric in list_of_metrics:
        ...
        if list_of_metrics.index(metric) == 0:
            print("Tree Depth Statistics : ", "Mean : ", mean_per_depth, " Variance:", variance_per_depth,
                  "Max Value: ", max_per_depth, " Min Value: ", min_per_depth)
        elif list_of_metrics.index(metric) == 1:
            print("R2 Value Statistics : ", "Mean : ", mean_per_depth, " Variance:", variance_per_depth, "Max Value: ",
                  max_per_depth, " Min Value: ", min_per_depth)
        elif list_of_metrics.index(metric) == 2:
            print("Validation MSE Statistics : ", "Mean : ", mean_per_depth, " Variance:", variance_per_depth,
                  "Max Value: ", max_per_depth, " Min Value: ", min_per_depth)

        elif list_of_metrics.index(metric) == 3:
            print("Training MSE Statistics : ", "Mean : ", mean_per_depth, " Variance:", variance_per_depth,
                  "Max Value: ", max_per_depth, " Min Value: ", min_per_depth)
...
def visualise_results(depth_range, number_of_chunks):
    ...
    mean_per_depth, variance_per_depth, max_per_depth, min_per_depth = nested_list_data_processing(
        validation_mse_nested)
...
def vanilla_tree(max_depth):
    [nested_array_of_performance, tree_depth_array, r2_score_array, mse_validation_array,
     mse_train_array] = test_across_depth_range(max_depth)
...
def bootstrapping_tree(max_depth):
    [nested_array_of_performance, tree_depth_array, r2_score_array, mse_validation_array,
     mse_train_array] = test_across_depth_range(max_depth, bootstrap=True)
    handler = [nested_array_of_performance, tree_depth_array, r2_score_array, mse_validation_array, mse_train_array]
    x = np.linspace(1, len(tree_depth_array), len(tree_depth_array))
    for i in handler:
        plot_results(i, x)
...
def normalising_tree(max_depth):
    [nested_array_of_performance, tree_depth_array, r2_score_array, mse_validation_array,
     mse_train_array] = test_across_depth_range(max_depth, normalisation=True)
    handler = [nested_array_of_performance, tree_depth_array, r2_score_array, mse_validation_array, mse_train_array]
    x = np.linspace(1, len(tree_depth_array), len(tree_depth_array))
    for i in handler:
        plot_results(i, x)
...
def cross_validation_tree(depth_range, number_of_chunks):
    rate, features = instantiate_data_set_with_api()
    [tree_depth_array, r2_score_array, mse_validation_array, mse_train_array] = run_test_depth_range_cross_valid(rate,
                                                                                                                 features,
                                                                                                                 depth_range,
                                                                                                                 number_of_chunks)
...
def validation_mse_plots(max_depth, number_of_chunks=5):
    ...
    y_var_raw = test_across_depth_range(max_depth)
    y_var_bootstrapped = test_across_depth_range(max_depth, bootstrap=True)
    y_var_normalised = test_across_depth_range(max_depth, normalisation=True)

    # FOR VALIDATION
    plt.plot(x, y_var_raw[3], label='Raw data')
    plt.plot(x, y_var_bootstrapped[3], label='Bootstrapped data')
    plt.plot(x, y_var_normalised[3], label='Normalised data')
    # rate, features = instantiate_data_set_with_api()
    # values=run_test_depth_range_cross_valid(rate,features,max_depth,number_of_chunks)[2]
    # mean_per_depth,variance_per_depth,max_per_depth,min_per_depth = nested_list_data_processing(values)
    # plt.plot(x,[ mean_per_depth], label='Cross-validated data')
    plt.legend()
    ...
    # FOR TRAINING
    plt.plot(x, y_var_raw[4], label='Raw data')
    plt.plot(x, y_var_bootstrapped[4], label='Bootstrapped data')
    plt.plot(x, y_var_normalised[4], label='Normalised data')
    rate, features = instantiate_data_set_with_api()
    [tree_depth_nested, r2_value_nested, validation_mse_nested, training_mse_nested] = run_test_depth_range_cross_valid(
        rate, features, max_depth, number_of_chunks)

    value_array = [tree_depth_nested, r2_value_nested, validation_mse_nested, training_mse_nested]

    mean_per_depth = nested_list_data_processing(value_array[2])[0]
